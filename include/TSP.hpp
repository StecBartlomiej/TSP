...
class CostMatrix {
public:
    // cost_matrix_t must be always square!
...
public:
    StageState(CostMatrix  m, std::vector<vertex_t> p = {},
            cost_t lb = 0) : matrix_(std::move(m)), unsorted_path_(std::move(p)), lower_bound_(lb) {};
